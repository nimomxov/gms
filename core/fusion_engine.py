...
class FusionResult:
    ...
    def confirmed_targets(self) -> list["FusedTarget"]:
        return [t for t in self.targets if getattr(t, "decision_tier", None) == "CONFIRMED"]

    @property
    def probable_targets(self) -> list["FusedTarget"]:
        return [t for t in self.targets if getattr(t, "decision_tier", None) == "PROBABLE"]

    @property
    def weak_targets(self) -> list["FusedTarget"]:
        return [t for t in self.targets if getattr(t, "decision_tier", None) == "WEAK"]

    @property
    def rejected_targets(self) -> list["FusedTarget"]:
        return [t for t in self.targets if getattr(t, "decision_tier", None) == "REJECTED"]
